...
import model_related.mhadatareader as p
...
def split_set_into_x_and_y(dictionary):
    ...
    to the time series data, and the other corresponding to the label for each time series (i.e novice or expert)
    """
    data = []
    labels = []
    for skill_level in dictionary:
        for surgery_type in dictionary[skill_level]:
            for t_series in dictionary[skill_level][surgery_type]:
                data.append(t_series)
                if "Novice".lower() in skill_level.lower():
                    labels.append(ProficiencyLabel.Novice.value)
                else:
                    labels.append(ProficiencyLabel.Expert.value)
...
def data_slicing(data, slice_len: int, label: ProficiencyLabel):
    return data, [label.value] * len(data)  # prevents window slicing
    slice_ratio = 0.9
    res = []
...
# Worked given 3 skill levels
# def slice_sequence_old(novices, intermeds, experts, slice_window) -> (
...
# Works given 2 skill levels - generalize later
def slice_sequence(novices, experts, slice_window) -> (
        np.ndarray, np.ndarray, np.ndarray, np.ndarray):
    x_novice, y_novice = data_slicing(novices, slice_window, ProficiencyLabel.Novice)
    x_expert, y_expert = data_slicing(experts, slice_window, ProficiencyLabel.Expert)

    x = np.array(x_novice + x_expert)
    y = np.array(y_novice + y_expert)
...
#
#     # read experts
#     experts = parser.read_data(f'{dir_name}/Experts/2014-07-03-Experts/')
...
# This works for 3 skill levels
# def prepare_folds_old(train_fold, valid_fold, test_fold, slice_window):
...
# This works for 2 skill levels
# def prepare_folds(train_fold, valid_fold, test_fold, slice_window):
#     x_novice, x_expert = train_fold
#     val_novice, val_expert = valid_fold
#     test_novice, test_expert = test_fold
#
#     x_train, y_train = slice_sequence(x_novice, x_expert, slice_window)
#     x_val, y_val = slice_sequence(val_novice, val_expert, slice_window)
#     x_test, y_test = slice_sequence(test_novice, test_expert, slice_window)
#
#     x_train, y_train = shuffle(x_train, y_train)
#     x_val, y_val = shuffle(x_val, y_val)
#     x_test, y_test = shuffle(x_test, y_test)
